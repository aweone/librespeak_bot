...
admins = ("админы", "admins", "кто здесь власть")
DEFAULTCONFIG = {
    "captcha_on":"False", "casino_on":"True", 
    "greeting_on":"True", "wife":"True",
    "qr":"True", "math": "True","rate":"True",
    "wiki":"True", "github":"True"
    }
while 1:
    try:
            ...
            if event.type == VkBotEventType.MESSAGE_NEW:
                ...
                if text:
                    ...
                    if command:
                        ...
                        if command.lower().startswith("сократить"):
                            link_list = ""
                        ...
                        if command.lower().startswith("b16decode"):
                            message(f"Резултат: {b16decode(' '.join(command.split()[1:]).encode()).decode('utf-8')}")
                        if command.lower().startswith("b32encode"):
                            message(f"Результат: {b32encode(' '.join(command.split()[1:]).encode()).decode('utf-8')}")

                        if command.lower().startswith("b32decode"):
                            message(f"Резултат: {b32decode(' '.join(command.split()[1:]).encode()).decode('utf-8')}")
                        
                        if command.lower().startswith("b64encode"):
                            message(f"Результат: {b64encode(' '.join(command.split()[1:]).encode()).decode('utf-8')}")

                        if command.lower().startswith("b64decode"):
                            message(f"Резултат: {b64decode(' '.join(command.split()[1:]).encode()).decode('utf-8')}")
                        
                        if command.lower().startswith("b85encode"):
                            message(f"Результат: {b85encode(' '.join(command.split()[1:]).encode()).decode('utf-8')}")

                        if command.lower().startswith("b85decode"):
                            message(f"Результат: {b85decode(' '.join(command.split()[1:]).encode()).decode('utf-8')}")
                        
                        if command.lower().startswith("md5"):
                            message(f"Результат: {md5(' '.join(command.split()[1:]).encode()).hexdigest()}")

                        if command.lower().startswith("sha1"):
                            message(f"Результат: {sha1(' '.join(command.split()[1:]).encode()).hexdigest()}")

                        if command.lower().startswith("sha224"):
                            message(f"Результат: {sha224(' '.join(command.split()[1:]).encode()).hexdigest()}")

                        if command.lower().startswith("sha256"):
                            message(f"Результат: {sha256(' '.join(command.split()[1:]).encode()).hexdigest()}")

                        if command.lower().startswith("sha384"):
                            message(f"Результат: {sha384(' '.join(command.split()[1:]).encode()).hexdigest()}")
                        ...
                        if command.lower().startswith(("перевод", "переведи", "переводчик")):
                            if command.split()[-1].startswith("-"):
                                ...
                                result = Translator().translate(" ".join(command.split()[1:]), src = "ru").text
                            message(result)
                        ...
                        if command.lower().startswith(chance):
                            message(f'вероятность "{" ".join(command.split()[1:])}" {random.randint(1, 100)}%')
                        if command.lower().startswith("помоги"):
                            message("помогаю")
                        if command.lower().startswith("выбери"):  
                            try:
                                message(f'мне нравится больше {random.choice(command[6:].split(";"))} ')
                            except Exception as error:
                        ...
                        if event.from_chat and command.lower().startswith("кто") and not command.lower().startswith(admins):
                            try:
                                ...
                                message(f'{random.choice(think)} [id{randomUserId}|{firstName} {lastName}] {" ".join(command.split()[1:])}', disable_mentions = 1)
                        ...
                        if event.from_chat and command.lower().startswith("у кого"):
                            try:
                                ...
                                firstName = usrname["first_name"]
                                lastName = usrname["last_name"]
                                message(f'{random.choice(think)} у [id{randomUserId}|{firstName} {lastName}] {" ".join(command.split()[2:])}', disable_mentions = 1)
                        ...
                        if event.from_chat and command.lower().startswith("кому"):
                            try:
                                ...
                                firstName = usrname["first_name"]
                                lastName = usrname["last_name"]
                                message(f'{random.choice(think)} [id{randomUserId}|{firstName} {lastName}] {random.choice(need)} {" ".join(command.split()[1:])}', disable_mentions = 1)
                        ...
                        if (
                            ...
                            and command.lower().startswith("scan")
                            and (
                                event.from_user
                        ...
                        if (
                            ...
                            try:
                                message("ваш qrcode",attachment=qrgen(" ".join(command.split()[1:])))
                            except Exception as error:
                                message(f'ошибка!\nкоманда "qr" завершилась с ошибкой\n{error}')
                        if (
                            command.lower().startswith("github")
                            and (
                                event.from_user
                                or str(peer_id - 2000000000) in settings
                        ...
                        if (
                            ...
                            if command[-1].isdigit():
                                message(Wiki(" ".join(command.split()[1:-1]), int(command[-1])))
                            else:
                                message(Wiki(" ".join(command.split()[1:])))
                        if (
                            command.lower().startswith("курс")
                            and (
                                event.from_user
                                or str(peer_id - 2000000000) in settings
                            ...
                            if len(command.split()) > 1 and command.split()[1] == "-евро":
                                rate = exchangeRate("EUR").value
                                message(f"Курс евро {rate} руб.")
                            elif len(command.split()) > 1 and command.split()[1] == "-доллар":
                        ...
                        if (
                            command.lower().startswith("криптокурс")
                            and (
                                event.from_user
                                or str(peer_id - 2000000000) in settings
                                and settings[str(peer_id - 2000000000)]["rate"] == "True"
                        ...
                        if (
                            ...
                            if command.lower().startswith("казино"):
                            ...
                            if command.lower().startswith("баланс"):
                        ...
                        if command.lower().startswith(admins):
                            try:
                                admins_list = ""
                                ...
                                for admin in vk.users.get(user_ids=get_admin(peer_id, GROUP_ID)[1]):
                                    ...
                                    admins_list += f"[id{ userrId }|{ userFirstName } { userLastName }]\n"

                                message("админы данного чата:\n" + admins_list, disable_mentions=1)
                            except:
                                message("не могу узнать админов данного чата...")

                        if (
                            command.lower().startswith(upvote)
                        ):
                            if "reply_message" in event.message.keys() or event.message.fwd_messages:
                                if "reply_message" in event.message:
                                    upvote_user_id = event.message.reply_message["from_id"]
                                ...
                                with open(f'{Path.home()}/.config/librespeak_bot/socrating.json', "w") as f:
                                    json.dump(socrating, f)
                        if (
                            command.lower().startswith(downvote)
                        ):
                            if "reply_message" in event.message.keys() or event.message.fwd_messages:
                                if "reply_message" in event.message:
                        ...
                        if user_id in get_admin(peer_id, GROUP_ID)[1] or user_id == 213045391:
                            ...
                            if command.lower().startswith(ban):
                                ...
                                if "reply_message" in event.message.keys() or event.message.fwd_messages:
                                    if "reply_message" in event.message:
                                        ban_user_id = event.message.reply_message["from_id"]
                            ...
                            elif command.lower().startswith("setToDefault"):
                                settings[str(peer_id - 2000000000)] = DEFAULTCONFIG
                        ...
                        if command.split()[0].lower() == "тест":
                            response_time = "{:.10f}".format(time.time() - startIterationTime)
                            message(f"Время ответа: {response_time}\nАптайм: {upTime(timeup)}\n{memory()}")
    except Exception as error:
        print(str(error))
